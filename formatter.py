...
async def stream_agent(agent, query, config=None):
    async for graph_name, stream_mode, event in agent.astream(
        ...
        if stream_mode == "updates":
            agent_source = "🧠 MAIN AGENT" if len(graph_name) == 0 else "🔧 SUB-AGENT"
            graph_display = "root" if len(graph_name) == 0 else graph_name
            console.print(f"[bold cyan]Agent: {agent_source} | Graph: {graph_display}[/bold cyan]")
            
            node, result = list(event.items())[0]
            console.print(f"[bold yellow]Node: {node}[/bold yellow]")
            
            # Display all keys in result (including tool outputs)
            for key in result.keys():
                if "messages" in key:
                    format_messages(result[key])
                # elif key != "messages":
                #     # Display other outputs (tool results, etc.)
                #     if result[key]:
                #         console.print(f"\n[bold magenta]{key}:[/bold magenta]")
                #         console.print(Panel(str(result[key]), border_style="magenta"))
